...
typedef struct {
	uint8_t	vbe_signature[4];			//db - VESA
	uint16_t vbe_version;				//dw 0200h
	phys_bytes oem_string_tr;			//dd
	uint8_t capabilities[4];			//4
	phys_bytes video_mode_ptr;			//dd ?
	uint16_t total_memory;				//dw
	uint16_t oem_software_rev;			//dw
	phys_bytes oem_vendor_name_ptr;		//dd
	phys_bytes oem_product_name_ptr;	//dd
	phys_bytes oem_product_rev_ptr;		//dd
	uint8_t reserved[222];				//db
	uint8_t oem_data[256];				//db
